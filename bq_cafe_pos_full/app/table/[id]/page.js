...
export default function TablePage({ params }) {
  ...
  async function voidOrder(orderId) {
    if (!orderId) return;
    // Nếu DB bạn không có status "void" -> đổi thành "cancelled"
  ...
  async function loadGroups() {
    ...
    if (error) {
      ...
      setGroups([]);
      // Gợi ý nguyên nhân hay gặp nhất
      setErrMsg(
        'Không tải được nhóm menu. Nếu bạn đang bật RLS cho menu_groups/menu_items, hãy tạo policy SELECT (public read) hoặc tắt RLS.'
      );
      return;
    }
  ...
  async function loadItems(groupId) {
    ...
    if (error) {
      ...
      setItems([]);
      setErrMsg(
        'Không tải được món. Nếu bạn đang bật RLS cho menu_items, hãy tạo policy SELECT (public read) hoặc tắt RLS.'
      );
      return;
    }
  ...
  async function loadOpenOrder() {
    ...
    // ✅ Không có open order => trả bàn về trống (đúng yêu cầu “bàn chưa chọn món thì trả về bàn trống”)
    if (!data?.length) {
      setOrder(null);
      setOrderItems([]);
      await setTableEmpty();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error('loadItems effect error:', e);
        if (alive) setItems([]);
      }
    })();
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
            ...
            {groups.length === 0 && (
              <div style={{ fontSize: 12, color: '#666' }}>
                Không có nhóm menu (hoặc bị chặn quyền RLS).
              </div>
            )}
          </div>
